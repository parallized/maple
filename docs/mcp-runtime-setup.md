...
The MCP server is the CLI's built-in `maple mcp` command. It only exposes the current process role, project directory and runtime root. Task state and reports continue to flow through the authenticated Server API; MCP is not an alternate authorization path.

Codex receives an invocation-scoped MCP configuration, and Claude receives `--mcp-config`. Maple does not write into `.codex`, `.claude`, `.gemini`, `.iflow` or `.windsurf`.
