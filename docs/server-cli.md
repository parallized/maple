...
- 构建产物位于 `apps/cli/dist/standalone`，其中包含入口和 WebUI；图片处理由 Bun 内置原生图像管线完成，无需额外原生依赖。
...
安装完成后运行 `maple` 可连接官方服务，或在 TUI 中选择「启动本地服务」；也可直接运行 `maple-local`。安装器会把 Server、WebUI、CLI 放在 `~/.maple`，不会在项目目录安装依赖。发布构建命令为：
