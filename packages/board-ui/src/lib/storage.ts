import type { McpServerConfig, Project, WorkerKind } from "../domain";
...
export function loadUiFont(): UiFont {
  try {
    const raw = localStorage.getItem(STORAGE_UI_FONT);
...
/** 基模(关键统领模型):新建任务默认执行的 Worker。 */
export function loadBaseWorker(): WorkerKind {
  try {
    const raw = localStorage.getItem(STORAGE_BASE_WORKER);
    if (raw && WORKER_KINDS.some((worker) => worker.kind === raw)) {
      return raw as WorkerKind;
    }
    return DEFAULT_BASE_WORKER;
  } catch {
    return DEFAULT_BASE_WORKER;
  }
}
