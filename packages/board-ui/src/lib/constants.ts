...
export type UiFont = "default" | "chill-round";
...
/** 基模:关键统领模型,新建任务默认交由基模执行。 */
export const DEFAULT_BASE_WORKER: WorkerKind = "claude";
...
export const DEFAULT_WORKER_CONFIGS: Record<WorkerKind, WorkerConfig> = {
  ...
  kimi: {
    executable: "kimi",
    runArgs: "--prompt --output-format stream-json",
    consoleArgs: "",
    probeArgs: "--version",
    dangerMode: false,
  },
  glm: {
    executable: "opencode",
    runArgs: "run --auto --format json --model zai-coding-plan/glm-5.2",
    consoleArgs: "",
    probeArgs: "--version",
    dangerMode: false,
  },
  ...
  gemini: {
    executable: "gemini",
    runArgs: "-p --output-format json --approval-mode=yolo",
    consoleArgs: "",
    probeArgs: "--version",
    dangerMode: false,
  },
