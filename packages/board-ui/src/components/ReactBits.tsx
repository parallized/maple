...
export function TiltedCard({
  ...
  scaleOnHover?: number;
  spotlightColor?: string;
  /** 关闭 3D 渲染上下文:卡片内有 background-clip:text 渐变文字时必须开启,
      否则 3D 上下文会把文字光栅化为纹理平面,文字裁剪失效(背景铺成矩形块)。 */
  flat?: boolean;
  onClick?: () => void;
  ...
  const x = useMotionValue(0);
  const y = useMotionValue(0);
  // 接近临界阻尼的硬弹簧:快速到位、干脆停住(restDelta 防止微小余振拖尾)。
  const springConfig = { stiffness: 320, damping: 30, restDelta: 0.001 };
