  ...
  return (
    <div className="flex gap-3">
      <div className="flex flex-col items-center pt-0.5">
        <div
          className="w-7 h-7 rounded-full border border-[color-mix(in_srgb,var(--color-base-content)_10%,transparent)] bg-(--color-base-100) flex items-center justify-center text-[12px] font-sans text-muted flex-none"
          aria-hidden="true"
        >
          {index}
        </div>
      ...
      <div className="flex-1 min-w-0 pb-1">
        <div className="flex items-center gap-2">
          ...
          <div className="text-[14px] font-sans font-medium text-(--color-base-content)">{title}</div>
        </div>
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between gap-2">
        <span className="text-[12px] text-muted font-sans">{title}</span>
        <button
          type="button"
      ...
      <pre className="m-0 text-[12px] leading-[1.55] font-mono text-(--color-base-content) bg-[color-mix(in_srgb,var(--color-base-200)_65%,transparent)] border border-[color-mix(in_srgb,var(--color-base-300)_45%,transparent)] rounded-[10px] px-2.5 py-2 whitespace-pre-wrap break-words select-all">
        {command}
      </pre>
...
function CommandSnippet({
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between gap-2">
        <span className="text-[12px] text-muted font-sans">{t("命令", "Command")}</span>
        <button
          type="button"
          className={`ui-btn ui-btn--xs ui-btn--ghost gap-1 flex-none ${copied ? "opacity-80" : ""}`}
          onClick={onCopy}
      ...
      <div className="ui-modal-panel" style={{ maxWidth: 760 }}>
        <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-[color-mix(in_srgb,var(--color-base-content)_6%,transparent)]">
          <div className="min-w-0">
            ...
            <p className="m-0 mt-1 text-[12px] text-muted font-sans opacity-75 truncate">{subtitle}</p>
          </div>
          <button
            type="button"
        ...
        <div className="ui-modal-body" style={{ paddingRight: 24, paddingLeft: 24 }}>
          <div className="flex flex-col gap-4">
            <Step index={1} title={t("准备 CLI", "Install CLI")} icon="mingcute:download-2-line">
              <p className="m-0 text-[12px] text-muted font-sans leading-relaxed">
                {cliReady
              ...
              {cliReady ? (
                <div className="mt-2 rounded-[10px] border border-[color-mix(in_srgb,var(--color-success)_25%,transparent)] bg-[color-mix(in_srgb,var(--color-success)_7%,transparent)] px-3 py-2 text-[12px] font-sans text-[var(--color-success)]">
                ...
                <div className="mt-3 flex flex-col gap-3">
                  {showNodeHint && nodeHint ? (
                    <div className="rounded-[10px] border border-[color-mix(in_srgb,var(--color-warning)_22%,transparent)] bg-[color-mix(in_srgb,var(--color-warning)_6%,transparent)] px-3 py-2 text-[12px] font-sans" style={{ color: "var(--color-warning)" }}>
            ...
            <Step index={3} title={t("刷新检测", "Refresh detection")} icon="mingcute:refresh-3-line">
              ...
              <div className="mt-3 flex items-center gap-2">
                ...
                {cliFound ? (
                  <span className="text-[12px] font-sans text-[var(--color-success)]">
                    <Icon icon="mingcute:check-line" className="text-[12px] mr-1 inline-block -translate-y-px" />
                    {t("已检测到 CLI", "CLI detected")}
                  </span>
                ) : (
                  <span className="text-[12px] text-muted font-sans opacity-70">{t("尚未检测到 CLI", "CLI not detected yet")}</span>
                )}
              </div>
            </Step>
