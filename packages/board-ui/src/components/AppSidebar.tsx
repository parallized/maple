...
export function AppSidebar({
  ...
  // 各 Worker 当前工作中的任务数(队列中 + 进行中),用于行尾 xN 计数。
  const activeCountByKind = new Map<string, number>();
  for (const project of projects) {
    for (const task of project.tasks) {
      if (task.status === "队列中" || task.status === "进行中") {
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto pr-0.5">
        ...
        <Reorder.Group as="div" axis="y" values={projectOrder} onReorder={onReorderProjects} className="flex flex-col gap-0.5">
          ...
          const isExecuting = project.tasks.some((task) => task.status === "队列中" || task.status === "进行中");
              ...
              {workers.map((worker) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`ml-0.5 flex-none text-[12px] font-semibold ${
                        activeCount > 0 ? "shimmer-metal" : "text-(--color-secondary)/60"
                      }`}
                    >
                      x{activeCount}
                    </span>
