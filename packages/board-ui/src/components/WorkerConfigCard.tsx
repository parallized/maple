...
import type { InstallTaskEvent, WorkerProbe } from "../platform/types";
...
export function WorkerConfigCard({
  ...
  const [guideTargetId, setGuideTargetId] = useState<InstallTargetId>(installTargetKind ?? "opencode");
  ...
  useEffect(() => {
    ...
    return platform.onInstallTaskEvent((payload) => {
      if (!payload) return;
  ...
  }, [platform, capabilities.canInstall]);

  // Determine what install targets are relevant for this worker
  const nativeTargetId = installTargetKind;
  const wslTargetId: InstallTargetId | null = installTargetKind ? (`wsl:${installTargetKind}` as InstallTargetId) : null;
  ...
  async function doInstall(targetIds: InstallTargetId[]) {
    if (!capabilities.canInstall || installing) return;
    ...
    try {
      ...
      await platform.installMcpSkills(options);
    } catch (error) {
  ...
  const runtimeRows: RuntimeRow[] = nativeTargetId
    ? [
        {
          id: nativeTargetId,
          runtime: "native",
  ...
  if (supportsWslRuntime && wslTargetId) {
    runtimeRows.push({
      id: wslTargetId,
      runtime: "wsl",
      runtimeLabel: "WSL",
  ...
  const guideRow = runtimeRows.find((row) => row.runtime === guideRuntime) ?? runtimeRows[0] ?? null;
  ...
  return (
    <div className={containerClass}>
      <div className={`${contentPaddingClass} flex flex-col gap-2.5`}>
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0 flex items-center gap-1.5 text-[12px] text-muted font-mono opacity-75">
            <Icon icon="mingcute:terminal-box-line" className="text-[14px] flex-none opacity-80" />
            ...
            {runtimeRows.map((row) => {
              ...
              return (
                <button
                  ...
                  title={text}
                >
                  <Icon icon={row.runtimeIcon} className="text-[14px] opacity-80" />
                  <span className="text-[12px] font-sans">{text}</span>
                  <Icon icon={statusIcon} className="text-[14px]" />
                </button>
              );
            })}
        ...
        {canReopenInstallWindow ? (
          <div className="flex justify-end">
            <button type="button" className="ui-btn ui-btn--xs ui-btn--ghost gap-1" onClick={() => setInstallWindowOpen(true)}>
              ...
              <span className="text-[12px] font-sans">{installing ? t("查看进度", "View progress") : t("查看日志", "View log")}</span>
            </button>
          </div>
        ) : null}
      </div>
