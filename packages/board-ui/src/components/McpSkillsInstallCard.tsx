...
import type { InstallTaskEvent } from "../platform/types";
...
export function McpSkillsInstallCard({
  ...
  const boardPlatform = usePlatform();
  const { capabilities } = boardPlatform;
  ...
  useEffect(() => {
    if (!capabilities.canInstall) return;
    return boardPlatform.onInstallTaskEvent((payload) => {
      if (!payload) return;
      ...
      if (payload.kind === "target_result") {
        const target = (payload.result ?? (payload as { target?: InstallTargetResult | null }).target ?? null) as InstallTargetResult | null;
        if (!target) return;
        setInstallTargetResults((prev) => ({ ...prev, [rawTargetId]: target }));
        return;
      }
    });
  }, [boardPlatform, capabilities.canInstall]);
  ...
  async function probeTargets() {
    ...
    try {
      const probes = await boardPlatform.probeInstallTargets();
  ...
  useEffect(() => {
    if (!capabilities.canInstall) return;
    void probeTargets();
  }, [capabilities.canInstall, probeToken]);
  ...
  const reportTargets = Object.values(installTargetResults).filter(
    (target): target is InstallTargetResult => Boolean(target)
  );
  ...
  if (!capabilities.canInstall) return null;
      ...
      <summary className="list-none cursor-pointer select-none p-3 lg:p-3.5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <div className="text-[14px] font-sans font-semibold text-(--color-base-content) truncate">
              {t("一键安装 MCP & Skills", "Install MCP & Skills")}
            </div>
            <div className="text-[12px] text-muted font-sans opacity-80 truncate">
      ...
      <div className="px-3 lg:px-3.5 pb-3.5 lg:pb-4">
        {capabilities.canInstall ? (
          <>
            <div className="flex flex-col gap-2.5">
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <p className="m-0 text-[12px] text-muted font-sans opacity-80 truncate">
                    {detectedSummary}
                  </p>
                </div>
                {showDetectButton ? (
                  ...
                  >
                    ...
                    <span className="text-[12px]">{t("刷新", "Refresh")}</span>
                  </button>
                ) : null}
              </div>
              ...
              {probeError ? (
                <div className="rounded-[10px] border border-[color-mix(in_srgb,var(--color-error)_30%,transparent)] bg-[color-mix(in_srgb,var(--color-error)_6%,transparent)] px-3 py-2">
                  <div className="flex items-center gap-2 text-[12px] font-sans text-(--color-base-content) opacity-85">
              ...
              <div className="flex items-center justify-between gap-2">
                <p className="m-0 text-[12px] text-muted font-sans opacity-80">
                  {supportsWslRuntime
                    ? t(
                ...
                <div className="flex items-center gap-2 flex-none">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      setInstalling(true);
                      try {
                        await boardPlatform.installMcpSkills({
                          codex: targets.codex,
            ...
            {installError ? (
              <div className="mt-2 rounded-[10px] border border-[color-mix(in_srgb,var(--color-error)_35%,transparent)] bg-[color-mix(in_srgb,var(--color-error)_7%,transparent)] px-3 py-2">
                ...
                <div className="mt-1 text-[12px] font-mono text-(--color-base-content) opacity-80 whitespace-pre-wrap break-words">
                  {installError}
                </div>
              </div>
            ) : null}

            {!installing && reportTargets.length > 0 ? (
                      ...
                      <summary className="list-none cursor-pointer select-none flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-[12px] text-muted font-sans opacity-80 truncate">{subtitleText}</div>
                          </div>
                        </div>
                        <Icon icon="mingcute:down-line" className="text-[16px] text-muted opacity-70 flex-none" />
                      </summary>

                      <div className="mt-2 text-[12px] font-sans text-(--color-base-content) opacity-80">
                        ...
                        {target.error ? (
                          <div className="mt-2 text-[12px] font-mono whitespace-pre-wrap break-words" style={{ color: "var(--color-error)" }}>
                            {target.error}
                          </div>
                        ) : null}
                      </div>
          ...
          <>
            ...
            <pre className="m-0 text-[12px] leading-relaxed font-mono text-(--color-base-content) bg-[color-mix(in_srgb,var(--color-base-200)_55%,transparent)] border border-[color-mix(in_srgb,var(--color-base-300)_55%,transparent)] rounded-[10px] px-3 py-2.5 whitespace-pre-wrap break-words">
              {command}
            </pre>

            <p className="m-0 mt-2 text-[12px] text-muted font-sans opacity-80">
              {t(
